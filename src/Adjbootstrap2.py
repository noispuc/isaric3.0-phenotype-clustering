...
for model bootstrapping and confidence intervals."""
...
def ci_boot(stats_null,estimator,alpha,X):
    """ generate confidence intervals from bootraped data"""
    stats=dict()
    for m in ['bic','aic','sabic','caic', "score"]:#change LL name to recall to make it work for LL
        ...
        stats[m]=ic_stats
    #percentile Boottrap CI
    # bias corrected boostrap CI
    # BCa not yet implemented as would require jackknife, that would increase processing time
    return stats
def bootstrap(
    estimator,
    X,
    ...
    for rep in tqdm_rep:
        ...
        ll_buffer.append(ll)
        aic_buffer.append(estimator_rep.aic(X))
        bic_buffer.append(estimator_rep.bic(X))        
        caic_buffer.append(estimator_rep.caic(X))
        sabic_buffer.append(estimator_rep.sabic(X))
        entropy_buffer.append(estimator_rep.entropy(X))
...
def blrt(null_model, alternative_model, X, Y=None, n_repetitions=30, random_state=42):
    ...
    #TODO add stats_df with conficence intervals, and estiamtes BIC etc.


    
    return b / n_repetitions, stats_null, stats_alternative
...
def blrt_sweep(
    ...
    for k in range(low, high):
        ...
        null_model.set_params(verbose=0) # added to check if it stops printing report
        alternative_model = clone(model)
        alternative_model.set_params(n_components=k + 1)        
        alternative_model.set_params(verbose=0) # added to check if it stops printing report
        p,stats_null, stats_alternative =blrt(
                null_model,
                alternative_model,
                X,
                Y=Y,
        ...
        if k==low:
            stats_null['k']=k
            boot_stats.append(stats_null)
            bic_ci=ci_boot(stats_null,null_model,0.05,X) #stats_null,estimator,alpha
            #TODO append k
            bic_ci=pd.DataFrame(bic_ci).T
            bic_ci['k']=k
            boot_ci_stats.append(bic_ci)
        # already computed stats
        stats_alternative['k']=k+1
        boot_stats.append(stats_alternative)     
        # confidence intervals
        bic_hat=alternative_model.bic(X)
        bic_ci=ci_boot(stats_alternative,alternative_model,0.05,X)
        #TODO append k
        bic_ci=pd.DataFrame(bic_ci).T
        bic_ci['k']=k+1
        boot_ci_stats.append(bic_ci)
    ...
    for m in ['bic','aic','sabic','caic', "score"]:
        fig, ax = plt.subplots()
        ...
        ax.set_title("Curve with Confidence Interval")
        ax.legend()
        plt.show()

    return df, boot_results, boot_stats, boot_ci_stats
