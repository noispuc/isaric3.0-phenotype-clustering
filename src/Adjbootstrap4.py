...
def blrt(null_model, alternative_model, X, Y=None, n_repetitions=30, random_state=42):
    ...
    #TODO add stats_df with confidence intervals, and estiamtes BIC etc.
...
def blrt_sweep(
    ...
    for k in range(low, high):
        ...
        if k==low:
            ...
            bic_ci['k']=k
            perc_conv=sum(stats_null['convergency_buffer'])/len(stats_null['convergency_buffer'])
            bic_ci['converngent']=perc_conv
            print('ncomp = '+str(k-1)+': '+str(perc_conv*100)+'% of '+str(n_repetitions)+' repetitions converged')
        ...
        bic_ci=pd.DataFrame(bic_ci).T
        bic_ci['k']=k+1
        perc_conv=sum(stats_alternative['convergency_buffer'])/len(stats_alternative['convergency_buffer'])
        bic_ci['converngent']=perc_conv
        boot_ci_stats.append(bic_ci)

        print('ncomp = '+str(k)+': '+str(perc_conv*100)+'% of '+str(n_repetitions)+' repetitions converged')
    ...
    for m in ['bic','aic','sabic','caic', "score",'entropy','relative_entropy']:
        ...
        ax.set_title("Percentile Confidence Interval")
        ax.legend()
        plt.show()
